...
string encrypt(string pt, vector<string> rkb,
	vector<string> rk)
{
	//Convert plaintext from string to bit
	pt = str2bin(pt);
	...
	// Final Permutation
	string cipher
		= bin2str(permute(combine, final_perm, 64));
	return cipher;
}

void decryptFile(std::string cipherFile)
{
	string key, pt;
	char buffer[8];
	ifstream file(cipherFile, ios::binary);
	file.read(buffer, 8);
	key = str2bin(string(buffer, 8));
	ofstream outFile("plaintext.txt", ios::binary);
	...
	{
		string cipher = encrypt(string(buffer, 8), rkb, rk);
		outFile << cipher;
	}

	file.close();
	outFile.close();
	ifstream ndFile("plaintext.txt", ios::binary);
	ofstream ndOutFile("temp.txt", ios::binary);
	char c;
	while (ndFile.get(c))
	{
		if (c != '\0') ndOutFile.put(c);
	}

	ndFile.close();
	ndOutFile.close();
	remove("plaintext.txt");
	rename("temp.txt", "plaintext.txt");
}
